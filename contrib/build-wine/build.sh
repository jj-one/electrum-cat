...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-cat-wine-builder-img \
    "$CONTRIB_WINE"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrum_build/windows/fresh_clone/electrum-cat"
...
docker run -it \
    --name electrum-cat-wine-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/wine64/drive_c/electrum-cat \
    --rm \
    --workdir /opt/wine64/drive_c/electrum-cat/contrib/build-wine \
    electrum-cat-wine-builder-img \
    ./make_win.sh
