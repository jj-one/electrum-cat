...
if ! which ${genisoimage} > /dev/null 2>&1; then
    mkdir -p /tmp/electrum-cat-macos
    cd /tmp/electrum-cat-macos
...
rm -rf /tmp/electrum-cat-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-cat-macos/image/
cp -r $1 /tmp/electrum-cat-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-CAT_uncompressed.dmg \
    /tmp/electrum-cat-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-CAT_uncompressed.dmg electrum-cat-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-CAT_uncompressed.dmg

echo "Done."
sha256sum electrum-cat-$VERSION.dmg
