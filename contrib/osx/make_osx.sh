...
PACKAGE=Electrum-CAT
GIT_REPO=https://github.com/catcoincore/electrum-cat
...
#cp -f "$DLL_TARGET_DIR"/libsecp256k1.*.dylib "$PROJECT_ROOT/electrum_cat" || fail "Could not copy libsecp256k1 dylib"
...
cp -f "$DLL_TARGET_DIR/libzbar.0.dylib" "$PROJECT_ROOT/electrum_cat/" || fail "Could not copy ZBar dylib"
...
cp -f "$DLL_TARGET_DIR/libusb-1.0.dylib" "$PROJECT_ROOT/electrum_cat/" || fail "Could not copy libusb dylib"


info "Installing requirements..."
export ELECTRUM_ECC_DONT_COMPILE=1
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: --only-binary scrypt \
...
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: --only-binary PyQt6,PyQt6-Qt6,cryptography,scrypt \
    --no-warn-script-location \
...
VERSION=$(python3 -c "import electrum_cat; print(electrum_cat.version.ELECTRUM_VERSION)")
...
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-cat-$VERSION-unsigned.dmg || fail "Could not create .DMG"
