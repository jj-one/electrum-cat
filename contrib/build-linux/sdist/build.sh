...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-cat-sdist-builder-img \
    "$CONTRIB_SDIST"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrum_build/sdist/fresh_clone/electrum-cat"
...
docker run -it \
    --name electrum-cat-sdist-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum-cat \
    --rm \
    --workdir /opt/electrum-cat/contrib/build-linux/sdist \
    --env OMIT_UNCLEAN_FILES \
    electrum-cat-sdist-builder-img \
    ./make_sdist.sh
